...
class Core
{
    ...
    public function see(string $selector, $webDriverBy = 'cssSelector'): static
    {
        ...
        if ( ! is_null($this->element)) {
            try {
                ...
                $this->navigation->cli(
                    'Could not find child element ' . $this->selector . '. Setting Element selected to null'
                );
                $this->element             = null;
        ...
        try {
            $this->element             = $this->navigation->webDriver->findElement(
                WebDriverBy::$webDriverBy($selector)
            );
            $this->visibilityOfElement = $this->element->isDisplayed();
        } catch (Exception $e) {
    ...
    public function amParentTo(?RemoteWebElement $remote_web_element = null): static
    {
        if ($remote_web_element instanceof RemoteWebElement) {
            try {
                $class = $remote_web_element->getAttribute('class');
            } catch (Exception $exception) {
                $class = 'Could not find className';
            }
            $this->navigation->cli('##### Selecting parent of element with class: ' . $class . ' ######');
            $this->element = $remote_web_element->findElement(WebDriverBy::xpath("./.."));

            if (is_null($this->element)) {
                $this->navigation->cli('##### WARNING: Parent not selected ######');

                return $this;
            }
            $class = $this->element->getAttribute('class');
            $this->navigation->cli('##### I am now parent with class: ' . $class . ' ######');
        } else {
            $this->navigation->cli(
                '##### WARNING: Parent not selected because no RemoteWebElement was passed as argument ######'
            );
            $this->element = null;
        }
    ...
    public function amGrandParentTo(?RemoteWebElement $remote_web_element = null, int $steps = 2): static
    {
        if ($remote_web_element instanceof RemoteWebElement) {
            $grandparent = $remote_web_element;
            $class       = $remote_web_element->getAttribute('class');
            $x           = 1;
            $this->navigation->cli(
                '##### Stepping up ' . $steps . ' times from element with class ' . $class . ' ######'
            );
            $xpath = '.';
            while ($x <= $steps) {
                $xpath .= '/..';
            ...
            $this->navigation->cli('##### Grandparent xpath selector: ' . $xpath . ' ######');
            $grandparent   = $grandparent->findElement(WebDriverBy::xpath($xpath));
            $this->element = $grandparent;

            $class = $this->element->getAttribute('class');
            $this->navigation->cli('##### I am now grandparent with class: ' . $class . ' ######');
        } else {
            $this->navigation->cli(
                '##### WARNING: Grandparent not selected because no RemoteWebElement was passed as argument ######'
            );
            $this->element = null;
        }
